...
while True:
    ...
    if lilypad_choice == 'y':
        ...
        while safe_count < 3: # Looping until 3 safe pads are reached
            ...
            # Multiple choice for retrieving backpack (using nested loops)
            attempts = 3  # Set number of attempts
            success = False
            
            for try_num in range(attempts):
              print(f"\nAttempt {try_num + 1}:")
              # Nested loop for different retrieval methods (distract, climb, reason)
              
              for method in ["distract", "climb", "reason"]:
              ...
              if success:
                break  # Exit outer loop if successful after any attempt

            if not success:
              print("The monkeys outsmarted you this time. All attempts failed!")
              
            # ... (rest of the code about river crossing and retrieving backpack)
            ...
            # User chooses left or right path
            choice = input("Will you go left (dark passage) or right (winding staircase)? ").lower()

            # Story continues based on user's choice
            ...
            if choice == "left":
              ...
              # Simple puzzle - guessing a number (modify difficulty as needed)
              found_key = False
              for attempt in range(3):
                guess = int(input("Enter a number between 1 and 10 to try and unlock the chest: "))
                if guess == 5:  # Modify the answer for different difficulty levels
                  found_key = True
              ...
              if found_key and choice == "right":  # User has key and chose right path
                  ...
                  # Add new content here for what the user finds inside the temple (puzzles, challenges, etc.)
                  ...
                  if choice == "A":
                    # Risky choice - touching the crystal
                    chance = random.randint(0, 1)  # 50% chance of success
                    if chance:
                      print("As you reach for the crystal, a warm light engulfs you. The symbols on the pedestal glow, and the room begins to shimmer. In a flash, you find yourself back in the Technos Institute lab, the VR headset deactivated.")
                      ...
                      print("As you touch the crystal, a surge of energy explodes outwards. The room shudders, and the symbols flare ominously. Uh oh...")
                    ...
                    # Add a simple logic puzzle here where the user has to decipher the symbols based on clues from the game
                    # If successful, continue with the escape (modify based on your desired ending)
                    print("You crack the code! The symbols on the pedestal rearrange, revealing a hidden passage behind it. You step through, ready to face what lies ahead...")

            elif found_key and choice == "left":  # User has key but chose left path
                print("You reach into your backpack, but there's no place to use the key here. Perhaps it unlocks something on the other side?")

            elif not found_key:  # User doesn't have the key
                print("The keyhole is empty. You must find a key to unlock this door.")
                  # Add hint or suggestion here (optional)
